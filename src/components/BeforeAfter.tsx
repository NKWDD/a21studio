...
export const BeforeAfter = ({ before, after, title }: { before: string; after: string; title: string }) => {
  ...
  return (
    <div className="space-y-3">
      <h3 className="font-block text-xl tracking-wider text-neon-cyan">{title}</h3>
      <div
        ref={ref}
      ...
      >
        ...
        <div className="absolute top-3 left-3 bg-background/85 px-2 py-1 font-block text-xs tracking-widest text-neon-pink">{t.gallery.before}</div>
        <div className="absolute top-3 right-3 bg-background/85 px-2 py-1 font-block text-xs tracking-widest text-neon-cyan">{t.gallery.after}</div>
        <div className="absolute top-0 bottom-0 w-1 bg-foreground shadow-neon-pink pointer-events-none" style={{ left: `calc(${pos}% - 2px)` }}>
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-neon-pink grid place-items-center shadow-neon-pink">
